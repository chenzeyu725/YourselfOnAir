...
- `taskStatus`：按任务状态过滤 dashboard 的任务统计（可选：`queued` / `running` / `done` / `failed`；支持逗号分隔多值，如 `taskStatus=queued,done`）。
- `documentStatus`：按文档状态过滤 dashboard 的文档统计（可选：`indexed` / `processing`；支持逗号分隔多值，如 `documentStatus=indexed,processing`）。
...
- `recentAuditOrder`：控制 `recentAuditLogs` 排序方向，`desc`（默认，最新在前）或 `asc`（最早在前）。
- `recentAuditOffset`：控制 `recentAuditLogs` 的跳过条数（非负整数，默认 `0`），可与 `recentAuditLimit` 组合实现窗口分页。
...
- 响应新增字段：
  ...
  - `scope.recentAudit`：回显当前生效的审计筛选条件（`limit/offset/order/action/method/actor/targetId/dateFrom/dateTo/groupLimit`）
  ...
  - `recentAuditByTarget`（审计日志按 `targetId` 聚合计数，忽略 `targetId` 为空的记录，基于当前筛选条件计算）
